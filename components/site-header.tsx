...
import { Rocket, Menu, X, ChevronDown, Sparkles, ScrollText, Heart, Coins, Users, Calendar, Sun, BookOpen, FileText, Gift, HeartPulse, Briefcase, User as User3 } from 'lucide-react'

type NavChild = { label: string; href: string; icon: typeof Gift; badge?: string }
type NavCategory = {
  id: string
  label: string
  icon: typeof Gift
...
const NAV_CATEGORIES: NavCategory[] = [
  {
    ...
    icon: Gift,
    children: [
      { label: '무료 타로 (오늘의 타로 & YES/NO)', href: '/free-tarot', icon: Sparkles, badge: 'FREE' },
      { label: '무료 만세력 (코스믹 스타맵)', href: '/#saju-form', icon: BookOpen, badge: 'FREE' },
  ...
  },
  {
    ...
    icon: ScrollText,
    children: [
      { label: '1:1 맞춤 정밀 사주 메인 종합 보고서 (올인원 패키지)', href: '/checkout', icon: FileText, badge: 'BEST' },
      { label: '2027 정미년 신년운세', href: '/newyear-2027', icon: Calendar, badge: '시즌' },
  ...
  },
  {
    ...
    icon: Briefcase,
    children: [
      { label: '테마: 재물 / 소득 / 자산운', href: '/theme/wealth', icon: Coins, badge: '추천' },
      { label: '테마: 커리어 / 이직 / 성공운', href: '/theme/career', icon: Briefcase, badge: '추천' },
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'FAQ & 1:1 문의하기', href: '/support', icon: ScrollText },
    ],
  },
]
...
export function SiteHeader() {
  ...
  return (
    <>
      <header className="no-print fixed top-0 left-0 right-0 z-40 border-b border-border bg-background/80 backdrop-blur-lg transition-shadow duration-300">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
              ...
              return (
                <div key={cat.id} className="group relative">
                  ...
                  <div className="invisible absolute left-0 top-full pt-1 opacity-0 transition-all group-hover:visible group-hover:opacity-100">
                    <div className="w-80 rounded-xl border border-border bg-popover p-2 shadow-xl">
                      {cat.children.map((child) => {
                        ...
                        return (
                          <Link
                            key={child.label}
                            href={child.href}
                            className={`flex min-h-[44px] items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors hover:bg-secondary/40 ${isActive(pathname, child.href) ? 'bg-amber-500/10 text-amber-500' : 'text-muted-foreground hover:text-foreground'}`}
                          >
                            <ChildIcon className="size-4 shrink-0 text-primary/70" />
          ...
          <button type="button" onClick={() => setOpen(true)} className="text-muted-foreground hover:text-foreground md:hidden" aria-label="메뉴 열기">
      ...
      >
        ...
        {/* Drawer panel */}
        <div
          className={`absolute right-0 top-0 flex h-full w-[85%] max-w-sm flex-col bg-background shadow-2xl transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
        >
          {/* Header: brand + close */}
          <div className="flex items-center justify-between border-b border-border p-5">
            ...
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="flex size-10 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-secondary/60 hover:text-foreground"
              aria-label="메뉴 닫기"
            >
              <X className="size-5" />
            </button>
                      ...
                      {cat.children.map((child) => {
                        ...
                        return (
                          <Link
                            ...
                            href={child.href}
                            onClick={() => setOpen(false)}
                            className={`flex min-h-[52px] items-center gap-3 rounded-lg px-3 py-3 pl-12 text-base font-medium transition-colors ${active ? 'bg-amber-500/10 text-amber-500' : 'text-muted-foreground hover:bg-secondary/40 hover:text-foreground'}`}
                          >
                            <ChildIcon className={`size-4 shrink-0 ${active ? 'text-amber-500' : 'text-primary/70'}`} />
                            <span className="flex-1 break-keep">{child.label}</span>
                            {child.badge && (
                              <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[0.65rem] font-bold ${BADGE_STYLES[child.badge] ?? 'bg-primary/15 text-primary'}`}>
                                {child.badge}
